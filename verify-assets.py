...
AIRBORNE = {
    'run':     [5, 11],                 # 12 帧双步循环：每步 6 拍，只在第 5/11 拍微腾空
...
LEG_TOP = 306           # 只看裙摆以下。旧值 270 会把各帧完全相同的裙摆算进来，
                        # 把「腿部姿态数」稀释成 2 个 —— 2026-09-23 修正
LEG_BOT = 365
...
STRIDE_SPREAD_MIN = 25  # 小步快跑：只判「腿有没有分开过」（旧值 45 是"大步跨"时代的）
...
def main():
    ...
    for clip, c in data.items():
        ...
        if m.get("flipForLeft"):
            ...
            if spreads:
                mx = max(spreads)
                # ⚠️ 2026-09-23：规格改成"小步快跑"（用户要求步幅缩小约 25%），
                # 所以跨度**不再要求大**，只要求"腿分开过"，别始终贴在一起。
                report.append(f"           两脚跨度 最大 {mx:.0f}% 角色高"
                              f"（小步快跑：只要求 ≥{STRIDE_SPREAD_MIN:.0f}%，"
                              f"不再要求大步）")
            ...
            if hs2:
                ...
                # ⚠️ 2026-09-23 规格反转：用户明确要求跑步时**身体必须稳定**
                #（"整体高度变化控制在 3% 以内"、"不允许因为跑步动作让角色视觉上突然变高"）。
                # 之前要求"身高起落 ≥6%"是按"靠整体上下弹表现跑步"写的 —— 那个做法被否掉了。
                # 现在压低的重量感由**腿的弯曲**承担，不由整体身高变化承担。
                report.append(f"           身高变化 {pct:.1f}%（规格要求 ≤3%：身体要稳，"
                              f"压低交给腿的弯曲而不是整体上下弹）")
                if pct > 3.0:
                    warns.append(
                        f"{clip}: 身高变化 {pct:.1f}%（{hspread}px）超过 3%"
                        f" → 身体在上下弹；用户明确要求「脑袋和上半身非常稳定」")
