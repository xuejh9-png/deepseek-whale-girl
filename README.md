...
- **`run`（跑步）素材做废了 —— 待返工。** 12 帧里腿部只有 **4 个水平姿态**、
  最大摆幅仅 **22 列**（画布宽 320）；腿部像素确实每帧在变，但变化全在
  **竖直方向**（跟着身体上下弹）—— 合起来就是"站着 + 弹两下"。
  运行时配上窗口横移 = **颠着平移**，不是跑。返工要求见 `docs/run-返工说明.md`。
  → 所以双击/右键/⌥⌘H 的「复位」目前观感一般，等素材返工后会自然。
