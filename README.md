...
- **`run`（跑步）返工后已可用。** 逐项实测（返工前 → 后）：
  ...
  对比图：`docs/run-返工前后对比.png`；返工要求：`docs/run-返工说明.md`。

  仍有两处**小瑕疵（不阻塞）**：
  1. 两次腾空落在第 5、10 帧，间隔是 **5 帧 / 7 帧**（两步循环应各 6 帧），
     对应步时 0.31s vs 0.44s —— 细看能感觉到一点点"跛"
  2. 接地帧脚底起落只有 **2px**（"压低"那一拍偏浅，目标 2~6px）
