...
- **`work`（坐着敲电脑）的循环段几乎看不出在动 —— 素材待返工。**
  这一组把运动做到了手指级，缩到实际显示尺寸后每帧只剩 **8~14 个屏幕像素**
  （对比 `idle` 的 900~4100px，差 **60 倍**），肉眼直接读成静止。
  方向已定：改成**头部 / 上半身的节奏性摆动**、幅度放大约 3~5 倍，不做手指细节。
  诊断见 `docs/pet-work-motion-diagnosis.png`，返工要求见 `docs/work-返工说明.md`。
  → 因此 `verify-assets.py` 目前会报 **1 条 FAIL**（10 支里 9 支通过），这是**预期结果**，
  不是脚本坏了。
