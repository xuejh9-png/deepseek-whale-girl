...
- **`run`（跑步）已用 AI 图生图重做，12 帧版上线。** 逐项对比：

  | 指标 | 第一版（外包） | 第二版（外包返工） | **现役（AI 生成 + 合成）** | 目标 |
  |---|---|---|---|---|
  | 腿部可区分形状 | 4（摆幅仅 22 列）| 9（摆幅 62 列）| **10（摆幅 118 列）** | ≥4 且摆幅 ≥60 |
  | 循环内宽度脉动 | 100→78%（−22%）| 100→78%（−22%）| **191~201（±2.6%）** | ≤12% |
  | 身高起落（压低感）| 2px（≈0）| 2px（≈0）| **55px（22%）** | ≥6% |
  | 腾空间隔 | 5/7 帧（不对称）| 5/7 帧（不对称）| **4 帧分 2 次，6/6 对称** | 相等 |
  | 帧数 / 帧率 | 12 @ 16fps | 12 @ 16fps | **12 @ 16fps（每步 6 拍）** | 步频 160~180/分 |

  12 帧 @16fps = 0.75 秒/循环 = **160 步/分**（真人跑步区间）。
  源姿势图与重新合成方法：`assets/源姿势/run/说明.md`。

  三条踩过的坑（都写进说明里了）：
  1. **外包两版都栽在"没人统一多帧尺寸"**（循环内横向缩放 76%~101%，脉动两次）。
     所以现在的分工是：**AI 只出姿势，尺寸一致性由 `make-sprite-sheet.py` 保证**。
  2. **抠背景必须用"从画面边缘灌水的连通域"**：她本人的白裙子/围裙/头饰和背景同色，
     阈值化会一起吃掉；而"每行取最左最右之间保留"会在角色分开的部位之间
     残留背景方块（2026-09-23 用户看到的"白色方块"就是这个，已修）。
  3. **时间分辨率不够会"僵硬"**：每步 4 拍（每帧停 91ms）偏卡；
     改成每步 6 拍（每帧 62ms）顺很多，且步频不变。
